...
describe('cluster', () => {
  ...
  test('can specify custom environment to cluster resource handler', () => {
    ...
    const app = nested.node.root as cdk.App;
    const stackTemplate = app.synth().getStackArtifact(nested.stackName).template;
    expect(stackTemplate.Resources.OnEventHandler42BEBAE0.Properties.Environment).toEqual({ Variables: { foo: 'bar' } });
  ...
  test('can place cluster handlers in the cluster vpc', () => {
    ...
    function assertFunctionPlacedInVpc(id: string) {
      expect(stackTemplate.Resources[id].Properties.VpcConfig.SubnetIds).toEqual([
  ...
  test('cluster security group is attached when adding self-managed nodes', () => {
    ...
    expect(stackTemplate.Resources.ClusterselfmanagedLaunchConfigA5B57EF6.Properties.SecurityGroups).toEqual([
  ...
  test('security group of self-managed asg is not tagged with owned', () => {
    ...
    expect(stackTemplate.Resources.ClusterselfmanagedInstanceSecurityGroup64468C3A.Properties.Tags).toEqual([
  ...
  test('cluster security group is attached when connecting self-managed nodes', () => {
    ...
    const app = stack.node.root as cdk.App;
    const stackTemplate = app.synth().getStackArtifact(stack.stackName).template;
    expect(stackTemplate.Resources.selfmanagedLaunchConfigD41289EB.Properties.SecurityGroups).toEqual([
  ...
  test('adding capacity creates an ASG without a rolling update policy', () => {
    ...
    const app = stack.node.root as cdk.App;
    const stackTemplate = app.synth().getStackArtifact(stack.stackName).template;
    expect(stackTemplate.Resources.ClusterASG0E4BA723.UpdatePolicy).toEqual({ AutoScalingScheduledAction: { IgnoreUnmodifiedGroupSizeProperties: true } });
  ...
  test('kubectl provider passes security group to provider', () => {
    ...
    const app = nested.node.root as cdk.App;
    const template = app.synth().getStackArtifact(nested.stackName).template;
